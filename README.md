...
> "这不是 RAG 问答，也不是拖拽式 DAG 工作流。这是一个标准的 ReAct Agent——LLM 在每一步自主决定调用哪个工具，工具在本地安全执行，推理循环持续到目标达成。同一个 /chat 接口，不同输入走完全不同的工具调用路径。DeepSeek 只知道工具的名字和参数定义，不知道内部 API 地址、认证方式、员工真实信息。"
